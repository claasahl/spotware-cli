const {marketOrderFromSpotPrices, stopOrderFromSpotPrices, limitOrderFromSpotPrices} = require("../../../build/services/local/order")
...
describe("marketOrderFromSpotPrices", () => {
    describe("actions", () => {
        test("should cancel order", async done => {
            ...
            stream.on("data", e => {
                if(e.type === "ACCEPTED") {
                    expect(stream.cancelOrder()).resolves
        ...
        test("should close order", async done => {
            ...
            stream.on("data", e => {
                if(e.type === "FILLED") {
                    expect(stream.closeOrder()).resolves
        ...
        test("should 'end' order (1)", async done => {
            ...
            stream.on("data", e => {
                if(e.type === "ACCEPTED") {
                    expect(stream.endOrder()).resolves
                } else if(e.type === "CANCELED") {
                    expect(e).toStrictEqual({...event, type: "CANCELED"})
                    done();
                }
...
describe("limitOrderFromSpotPrices", () => {
    describe("actions", () => {
        test("should cancel order", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "SELL", volume: 2, enter: 6, spots })
            const event = {timestamp: 1};
            stream.on("data", e => {
                if(e.type === "ACCEPTED") {
                    expect(stream.cancelOrder()).resolves
        ...
        test("should close order", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "SELL", volume: 2, enter: 4.5, spots })
            const event = {timestamp: 2, exit: 2, profitLoss: 6};
            stream.on("data", e => {
                if(e.type === "FILLED") {
                    expect(stream.closeOrder()).resolves
                } else if(e.type === "CLOSED") {
            ...
            spots.push({ type: "BID_PRICE_CHANGED", timestamp: 0, bid: 4 })
            spots.push({ type: "BID_PRICE_CHANGED", timestamp: 1, bid: 5 })
            spots.push({ type: "ASK_PRICE_CHANGED", timestamp: 2, ask: 2 })
        })
        test("should close order exactly once", async done => {
        ...
        test("should 'end' order (1)", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "BUY", volume: 2, enter: 6, spots })
            const event = {timestamp: 1};
            stream.on("data", e => {
                if(e.type === "ACCEPTED") {
                    expect(stream.endOrder()).resolves
        ...
        test("should 'end' order (2)", async done => {
            ...
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "BUY", volume: 2, enter: 6, spots })
            const event = {timestamp: 2, exit: 10, profitLoss: 8};
            stream.on("data", e => {
                if(e.type === "FILLED") {
                    expect(stream.endOrder()).resolves
                } else if(e.type === "CLOSED") {
    ...
    describe("order type: BUY", () => {
        ...
        test("expire order asap", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "BUY", volume: 2, enter: 6, expiresAt: 15, spots })
            const event = { type: "EXPIRED", timestamp: 15 }
            stream.on("data", e => {
                if(e.type === "EXPIRED") {
                    expect(e).toStrictEqual(event);
        ...
        test("take profit (1)", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "BUY", volume: 2, enter: 6, spots, takeProfit: 11, stopLoss: 2 })
            const event = { type: "CLOSED", timestamp: 6, profitLoss: 10, exit: 11 }
            stream.on("data", e => {
                if(e.type === "CLOSED") {
                    expect(e).toStrictEqual(event);
    ...
    describe("order type: SELL", () => {
        test("create order asap", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "SELL", volume: 2, enter: 4, spots })
            const event = {type: "CREATED", timestamp: 1}
            stream.on("data", e => {
                if(e.type === "CREATED") {
                    expect(e).toStrictEqual(event);
        ...
        test("expire order asap", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "SELL", volume: 2, enter: 4, expiresAt: 15, spots })
            const event = { type: "EXPIRED", timestamp: 15 }
            stream.on("data", e => {
                if(e.type === "EXPIRED") {
                    expect(e).toStrictEqual(event);
        ...
        test("take profit (1)", async done => {
            const symbol = Symbol.for("abc")
            const spots = new SpotPricesStream({ symbol })
            const stream = await limitOrderFromSpotPrices({ id: "1", symbol, tradeSide: "SELL", volume: 2, enter: 4, spots, takeProfit: 2, stopLoss: 11 })
            const event = { type: "CLOSED", timestamp: 6, profitLoss: 4, exit: 2 }
            stream.on("data", e => {
                if(e.type === "CLOSED") {
                    expect(e).toStrictEqual(event);
