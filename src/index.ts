...
import ms from "ms";
import fs from "fs";
import {connect, write, ProtoOAPayloadType, ProtoPayloadType} from "@claasahl/spotware-adapter";
...
socket.on("PROTO_MESSAGE.2113", message =>
  fs.writeFileSync("./assets.json", JSON.stringify(message, null, 2), {encoding: "utf8"})
);
socket.on("PROTO_MESSAGE.2115", message =>
  fs.writeFileSync("./symbols.json", JSON.stringify(message, null, 2), {encoding: "utf8"})
);
socket.on("PROTO_MESSAGE.2154", message =>
  fs.writeFileSync("./assetClasses.json", JSON.stringify(message, null, 2), {encoding: "utf8"})
);
socket.on("PROTO_MESSAGE.2161", message =>
  fs.writeFileSync("./categories.json", JSON.stringify(message, null, 2), {encoding: "utf8"})
);
